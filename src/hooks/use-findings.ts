...
export const useFindings = () => {
  ...
  const [findingsData, setFindingsData] = useState<Finding[]>(mockFindingsData);
  ...
  const fetchFindings = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "AWS security findings have been refreshed"
      });
    } catch (error) {
      console.error("Error fetching AWS data:", error);
      toast({
        title: "Error",
        description: "Could not fetch AWS security findings",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
